...
import { FocusArea, RefineMode } from '../../../src/types';
...
const FocusAreaButton: React.FC<FocusAreaButtonProps> = React.memo(({ opt, isSelected, onClick }) => (
    <button 
        ...
        title={opt.title} 
        className={`
            flex items-center justify-center gap-2.5 p-3 text-sm font-medium rounded-lg border transition-all duration-200 relative
            ${isSelected 
                ? 'bg-purple-600 text-white border-purple-400 shadow-[0_0_15px_rgba(168,85,247,0.4)] ring-1 ring-purple-400' 
                : 'bg-gray-800/40 border-gray-700 text-gray-400 hover:bg-gray-700 hover:border-gray-500 hover:text-gray-200'
            }
        `}
    >
    ...
    return (
        <div>
            <label className="block text-sm font-medium text-gray-300 mb-3">
                Focus Areas <span className="text-gray-500 font-normal text-xs ml-2">(Select multiple)</span>
            </label>
