...
export function TopAppBar({ currentScreen, setCurrentScreen, versionCount = 0, showToast, wordCount = 0, isMobile = false }: TopAppBarProps) {
    ...
    <>
      <header className={`sticky top-0 left-0 w-full z-50 flex flex-col px-4 md:px-6 ${isMobile ? 'py-2' : 'py-3'} bg-surface/80 backdrop-blur-xl border-b border-outline-variant/10 ${isMobile ? 'gap-1' : 'gap-3'} overflow-visible`}>
        <div className="flex justify-between items-center w-full gap-4">
          <div className={`flex items-center gap-2 md:gap-3 cursor-pointer shrink-0 ${isMobile ? 'w-full justify-center' : ''}`} onClick={() => setCurrentScreen('welcome')}>
        ...
        <div className={`flex items-center gap-2 md:gap-4 flex-wrap overflow-visible ${isMobile ? 'justify-center w-full' : ''}`}>
          {!isMobile && <ProjectManager showToast={showToast} />}
          {currentScreen === 'workspace' && (
            <>
              ...
              <button className="flex items-center gap-2 px-2 md:px-4 py-1 rounded-full bg-surface-container-highest ghost-border hover:bg-surface-container-high transition-colors shrink-0">
                <span className="material-symbols-outlined text-[10px] md:text-sm">history</span>
                <span className="font-label text-[8px] md:text-xs uppercase tracking-wider">v1.0.{versionCount}</span>
              </button>
            </>
          )}
        </div>
